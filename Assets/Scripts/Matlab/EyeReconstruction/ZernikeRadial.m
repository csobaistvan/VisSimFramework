function radial = ZernikeRadial(r,n,m)
    ...
    if n == m
        radial = r.^n;
    elseif n - m == 2
        radial = n*ZernikeRadial(r,n,n)-(n-1)*ZernikeRadial(r,n-2,n-2);
    else
        H3 = (-4*((m+4)-2)*((m+4)-3)) / ((n+(m+4)-2)*(n-(m+4)+4));
        H2 = (H3*(n+(m+4))*(n-(m+4)+2)) / (4*((m+4)-1))  +  ((m+4)-2);
        H1 = ((m+4)*((m+4)-1) / 2)  -  (m+4)*H2  +  (H3*(n+(m+4)+2)*(n-(m+4))) / (8);
        radial = H1*ZernikeRadial(r,n,m+4) + (H2+H3 ./ r.^2).*ZernikeRadial(r,n,m+2);
