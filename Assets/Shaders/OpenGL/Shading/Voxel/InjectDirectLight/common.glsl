...
#include <Shaders/OpenGL/Shading/LightTypes/light_types.glsl>
...
{
    // Move the voxel position forward to avoid shadow mapping errors
    SurfaceInfo surfaceWithOffset = surface;
    surfaceWithOffset.position += surface.normal * sRenderData.fVoxelSize * 1.0;

    // Calculate the lighting contribution
    return computeLight(surfaceWithOffset, material, 0.0, 1.0, 0.0, 0.0);
}
...
void accumulateLight(const ivec3 gridCoords, const vec4 light)
{
    ...
    const vec4 outValue = vec4(light.rgb + prevRadiance, 1.0);
