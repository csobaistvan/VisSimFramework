...
void main()
{
    ...
    const float rd_c = farBlurRadius(dilatedBlurRadius(gl_FragCoord.xy));
    ...
    for (int t = -KERNEL_RADIUS_HORIZONTAL, s = 0; t <= KERNEL_RADIUS_HORIZONTAL; ++t, ++s)
    {
        ...
        const float rd_s = farBlurRadius(dilatedBlurRadius(texelUV * sRenderData.vResolution));
        const float r_s = texel.a;
        const vec3 c_s = texel.rgb;

        // Calculate the weight of the sample
        const float weight = farBlurWeight(calcSampleDist(t, gatherRadius), r_c, rd_c, r_s, rd_s) * farFieldness_c;
        ...
        for (int c = 0; c < KERNEL_COMPONENTS; ++c)
        {
            ...
            {
                blurSum[c][ch] += c_s[ch] * weight * kernel + c_c[ch] * (1-weight) * kernel * farFieldness_c;
            }
        }
    }
